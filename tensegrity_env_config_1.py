...
from mujoco_physics_engine.tensegrity_mjc_simulation_config_1 import TensegrityMuJoCoSimulator as Simulator # Adjust import if needed

def debug_print(message, filename="tensegrity_env_config_1.py", debug_enabled=False):
...
class TensegrityEnv(gym.Env):
    def __init__(self, obs_dim=None, visualize=False, obs_mode: str = "tier2", debug_enabled=False):
        ...
        debug_print(f"Simulator.__init__ called with visualize={visualize}", "tensegrity_env_config_1.py", debug_enabled)

        # Define observation spaces
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(self.sim.obs_dim,), dtype=np.float32)
        
        # Define action spaces
        self.num_actuators = 6  # 12 actuated cables
    ...
    def render(self, mode='human'):
        ...
        try:
            ...
            debug_print(f"Render failed: {e}", "tensegrity_env_config_1.py", self.debug_enabled)
            return None
