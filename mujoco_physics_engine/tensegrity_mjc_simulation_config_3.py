...
class TensegrityMuJoCoSimulator(AbstractMuJoCoSimulator):
    ...
    def __init__(self,
                 ...
                 render_fps: int = 20,
                 num_actuated_cables: int = 18, #was 12
                 num_rods: int = 3,
        ...
        self.actuated_ids = [0, 1, 2, 3, 4, 5, 15, 16, 17, 18, 19, 20, 30, 31, 32, 33, 34, 35]  # All vertex-to-bar cables
        ...
        self.obs_dim = (141 if self.obs_mode == "tier2" else 141) if obs_dim is None else obs_dim
        ...
        self.strain_tendon_ids = list(range(min(27, self.n_cables)))
        # IMU (rods) geom names present in XML sensors
        self.imu_geom_names = ["t1_r01", "t1_r23", "t1_r45", "t2_r01", "t2_r23", "t2_r45","t3_r01", "t3_r23", "t3_r45"]
    ...
    def get_observation_tier2(self):
        ...
        parts = [
            cable_lengths_norm,          # 12 --> 18
            cable_rates_norm,            # 12 --> 18
            prev_action,                 # 12 --> 18
            strain_exts,                 # 18 --> 27
            imu_grav,                    # 18 -->27
            imu_ang_norm,                # 18 --> 27
        ...
        # Ensure exact 96 dims #maybe 141 now
        if obs.shape[0] != 141:
            if obs.shape[0] < 141:
                obs = np.pad(obs, (0, 141 - obs.shape[0]))
            else:
                obs = obs[:141]
