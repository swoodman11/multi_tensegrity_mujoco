...
def create_tier2_observation_plots(obs_np, timestamp, save_plots=False):
    ...
    # Define observation groups for Tier2
    groups = {
        "Cable Lengths (Normalized)": (0, 6),       # 6 cables (1 robot)
        "Cable Length Rates": (6, 12),              # 6 velocity measurements
        "Previous Actions": (12, 18),               # 6 previous actuator commands
        "Strain Extensions": (18, 27),              # 9 tendons (6 actuated + 3 structural)
        "IMU Gravity Vectors": (27, 36),            # 9 values (3 IMUs × 3 axes)
        "IMU Angular Velocities": (36, 45),         # 9 values (3 IMUs × 3 axes)
        "COM Linear Velocity": (45, 48),            # 3 values (x, y, z)
        "COM Angular Velocity": (48, 51)            # 3 values (roll, pitch, yaw)
    }

# Total observation dimension: 51D
...
def create_detailed_cable_plots(obs_np, timestamp, save_plots=False, obs_mode="tier2"):
    ...
    if obs_mode == "tier2":
        ...
        cable_lengths = obs_np[:, 0:6]
        cable_rates = obs_np[:, 6:12] 
        prev_actions = obs_np[:, 12:18]
...
num_actuators = getattr(env, 'num_actuators', 6)
